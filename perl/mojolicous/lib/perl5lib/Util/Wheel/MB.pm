...
sub new {  
  ...
  if ($args->{reader} && $args->{InputEvent} ) { 
    ...
    unless($reader) {
      cluck "can not Util::IPC::MB->create($args->{reader}, fifo)";
      return undef;
    }
  ...
  if ($args->{writer}) { 
    ...
    unless($writer) {
      cluck "can not Util::IPC::MB->create($args->{writer}, fifo)";
      return undef;
    }
...
=head1 NAME
  ...
  Util::Wheel::MB  - wheel for fifo-base mbox reading and writing
...
=head1 SYNOPSIS
  ...
  sub on_start {
    $_[HEAP]{wheel} = Util::Wheel::MB->new(
      reader     =>  'test',
      writer     =>  'test',
      InputEvent =>  'on_data',
      ErrorEvent =>  'on_error',
