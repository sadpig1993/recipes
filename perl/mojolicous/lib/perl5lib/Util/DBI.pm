...
sub rollback     { return shift->{dbh}->rollback() or confess "can not rollback"; }
...
#
# 通用数据库查找功能
# $self->select_sql($sql, [val1, val2], \%option )
#
# $sql  : sql
# $val  : 域值
# %option : { rs => 1, sth => 1/0 }
...
#
# 通用数据库查找功能
# $self->select_page($table_name, ['fld_x', 'fld_y'], { fld_a => '1', fld_b => '2'}, ['ord1', 'ord2'], ['grp1', 'grp2']
#                                   {cur_page=>$cur_page, page_size => $page_size}, \%option )
#
# $table  : 表名
# $cond   : 条件域以及域值
# %option : { rs => 1, sth => 1/0 }
...
sub select_page {
    ...
    my ($self, $table, $need, $cond, $ord, $grp, $page, $option) = @_;

    confess "no connection to database" unless $self->{dbh};
    $table or croak "error";
    
    my $cur_page = $page->{cur_page};
    my $page_size = $page->{page_size};
    
    my $start = ($cur_page - 1) * $page_size + 1;
    ...
    # 组sql语句
    if (@fld) {
        $sql = 'select * from (select ' . join(',', @$need) . " ,rownumber() over() as rowid from $table where " . join(" and ", map { "$_ = ?" } @fld);
        $cnt = 'select count(*) from (select ' . join(',', @$need) . " from $table where " . join(" and ", map { "$_ = ?" } @fld);
    } else {
        $sql = 'select * from (select ' . join(',', @$need) . " ,rownumber() over() as rowid from $table ";
        $cnt = 'select count(*) from (select ' . join(',', @$need) . " from $table ";
    }
    $sql .= ' group by ' . join(',', @$grp), $cnt .= ' group by ' . join(',', @$grp) if $grp;
    $sql .= ' order by ' . join(',', @$ord), $cnt .= ' order by ' . join(',', @$ord) if $ord;
    $sql .= ' with rs for update' if $option->{rs};
    $cnt .= ')';
    $sql .= ") where rowid>= $start and rowid < $end";
    warn "select sql[$sql] val[@val]" if $ENV{PLUGIN_DB_DEBUG};
    
    # prepare
    my $sth = $self->{dbh}->prepare_cached($sql) or confess "can not prepare_cached[$sql]";
    my $sth_cnt = $self->{dbh}->prepare_cached($cnt) or confess "can not prepare_cached[$sql]";
    # 执行
    $sth->execute(@val) or confess "can not execute[$sql][@val]";
    $sth_cnt->execute(@val) or confess "can not execute[$cnt][@val]";
 
    # 返回statement 
    if ( $option->{sth} ) {
        return { sth => $sth, cnt => $sth_cnt};
    }
